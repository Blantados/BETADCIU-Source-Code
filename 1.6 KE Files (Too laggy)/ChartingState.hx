...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSectionUI():Void
	{
		...
		check_bfAltAnim = new FlxUICheckBox(60, 340, null, null, "BF Alternate Animation", 100);
		...
		var refresh = new FlxButton(10, 60, 'Refresh Section', function() {
			...
			check_bfAltAnim.checked = section.bfAltAnim;
		});
		...
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case "BF Alternate Animation":
					getSectionByTime(Conductor.songPosition).bfAltAnim = check.checked;
			}
		}
		...
		*/
		...
		if (weird != null)
		{
			if (lastUpdatedSection != getSectionByTime(start,true))
			{
				...
				check_bfAltAnim.checked = weird.bfAltAnim;
			}
		}
	...
	private function newSection(lengthInSteps:Int = 16,mustHitSection:Bool = false,altAnim:Bool = true,bfAltAnim:Bool = true):SwagSection
		{
			...
			for (daSection1 in 0..._song.notes.length)
				{
					...
					newSong.push(newSection(16,_song.notes[daSection1].mustHitSection,_song.notes[daSection1].bfAltAnim));
				}
